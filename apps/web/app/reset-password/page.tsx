...
import { errorMessage, usePasswordResetConfirmMutation } from '@/components/auth/auth-query';
...
function ResetPasswordForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError('');
    try {
      await resetConfirmMutation.mutateAsync({ resetToken: token, password });
  ...
  return (
    <AuthShell title="Nova senha" description="Confirme token e defina senha." links={[{ href: '/login', label: 'Voltar para login' }]}>
      <form onSubmit={submit} className="grid gap-4">
        ...
        <Button type="submit" disabled={resetConfirmMutation.isPending || !token || !password}>{resetConfirmMutation.isPending ? 'Salvando...' : 'Salvar senha'}</Button>
      </form>
    </AuthShell>
  );
}
